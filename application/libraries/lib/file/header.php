...
/*
	@abstract Read file header (size, mimetype etc) of a remote file e.g. before downloading it
	@example  print_r(hub::file_header('http://www.example.com'))
	@param    string $url
	@param    int $timeout [default=10]
	@return   array [status,reason,_request,_response,etc.]
...
if(basename(__FILE__)==basename($_SERVER['PHP_SELF'])){
  echo '<pre>';
  print_r(file_header('http://www.example.com/'));
}

function file_header($url,$timeout = 10) {
	...
	else $head .= 'User-Agent: Mozilla (OS) Browser'."\r\n"; // MAYBE YOU WANT TO EDIT THE User-Agent?
